...
describe('TC smoke', function(){
    ...
    it('Check Navbar and login', () => {
        ...
        cy.contains('.nav-item', 'Помощь')
            .should('be.visible').click().then( help => {
                cy.wrap(help)
                    .contains('[class="dropdown-item"]','Как оформить заказ')
                    .should('be.visible')
                cy.wrap(help)
                    .contains('[class="dropdown-item"]','Гарантия')
                    .should('be.visible')
                });
    ...
    it('Contains element in homepage', () => {
        cy.login(name, password, sourseUrl)
        cy.visit(`${sourseUrl}`)
        cy.contains('b', 'Категории товаров')
            .should('be.visible')


        HomePage.getCategory("Холодильники")
            .should('be.visible')
        HomePage.getCategory("Телевизоры")
            .should('be.visible')
        HomePage.getCategory("Музыкальные центры")
            .should('be.visible')
        HomePage.getCategory("Ноутбуки")
            .should('be.visible')
        ...
        Category.getAllItems().each(elem => {
            ...
            Category.getAllItems().first().then(tag => {
                    cy.wrap(tag).find('h5[class="fw-bolder"]').then(getText => {
                        ...
                        cy.wrap(tag)
                            .contains('Перейти').click({force: true})
                        cy.get('h5[class="card-title"]')
                            .find('b')
                            .should('have.text', text)
                        HomePage.getCartInNavbar('0').
                            should('be.visible')
                        cy.get('a[class="btn btn-primary"]').click({forse:true}).then(()=>{
                            HomePage.getCartInNavbar('1')
                                .should('be.visible')
                        }) 
                    }) 
            })
    ...
    it('Cart', () => {
        ...
        cy.get('.form-control')
            ...
            .type(2)
        cy.get('tr')
            .find(':nth-child(3) > form')
            .contains('Обновить').click()
